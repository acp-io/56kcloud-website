...
async function takeScreenshot(page: Page, pageInfo: PageInfo, outputPath: string): Promise<void> {
...
async function getPages(): Promise<PageInfo[]> {
  ...
  return pages.filter((page) => page.locale === 'en').filter((page) => !page.path.startsWith('/en/blog/'))
}
